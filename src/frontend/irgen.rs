...
use crate::ir::{
    self, Block, CastOp, ConstantValue, Context, FloatBinaryOp, FloatCmpCond, Func, Global, Inst,
    IntBinaryOp, IntCmpCond, TargetInfo, Ty, Value,
};
        ...
        match &exp.kind {
            ...
            ExpKind::Unary(op, exp) => match op {
                ...
                UnaryOp::Not => {
                    ...
                    let true_val = Value::i1(&mut self.ctx, true);
                    let xor =
                        Inst::int_binary(&mut self.ctx, operand, true_val, IntBinaryOp::Xor, i1_ty);
                    curr_block
                        .push_back(&mut self.ctx, xor)
            ...
            ExpKind::FuncCall(FuncCall { ident, args }) => {
                ...
                curr_block
                    .push_back(&mut self.ctx, call)
                    .expect("Failed to insert call instruction");
